...
# ╔═╡ 54ac9084-ebf1-493b-b6ff-d02805a34c10
begin
	...
	# calculate vi (v_max is known, just need to calculate the right half in the bracket parentheses)
	
	#1.14.13.39: arginine: 73.1
	#73.1/(1+73.1-1) = 1
	#vi = v_max*1

	#3.5.3.1: arginine: 0.165 ***I chose the one in homo sapiens
	#0.165/(1+0.165-1) = 1
	#vi = v_max*1

	#4.3.2.1: fumarate: 0.0915, arginine: 0.0852
	#vi = v_max*0.04225462
	#however both are products and we are only concerned with reactants, so set the value to 1

	#6.3.4.5: atp: 11.9, aspartate: 96.7
	#11.9*96.7/[(11.9+1)(96.7+1)-1] = 0.91376367
	#vi = v_max*0.91376367

	#2.1.3.3: ornithine: 0.0119 ***I chose the one in E Coli
	#0.0119/(1+0.0119-1) = 1
	#vi = v_max*1
	...
	flux_bounds_array2[1,2] = 203.0*E2[1]*0.9138
	flux_bounds_array2[2,2] = 34.5*E2[2]*1
	flux_bounds_array2[3,2] = 249.0*E2[3]*1
	flux_bounds_array2[4,2] = 88.1*E2[4]*1
	flux_bounds_array2[5,2] = 13.7*E2[5]*1
	flux_bounds_array2[6,2] = 13.7*E2[6]*1
	...
	# what is the index of the urea flux?
	idx_b₄2 = findall(x->x=="b4", rna)[1]
	c_vector2[idx_b₄2] = -1

	# compute the flux -
	result_case_2 = lib.flux(S,flux_bounds_array2,species_bounds_array2,c_vector2);

	# show -
	nothing
end

# ╔═╡ d6a7a43f-a8c8-4512-b3a2-37b1d2d2ac94
# check:
with_terminal() do
...
# ╔═╡ 0693f717-cdec-4786-afa6-20e0872388d3
let
